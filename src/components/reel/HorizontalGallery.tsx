      ...
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        ...
        >
          ...
          {projects.map((project, i) => (
            <Link
              key={project.slug}
              href={`/reel/work/${project.slug}`}
              className="group relative block shrink-0 overflow-hidden bg-white/[0.03]"
              style={{
                height: `${FRAME_VH}vh`,
                width: `${FRAME_VH * mediaInfo(project.poster).aspect}vh`,
              }}
            >
              <Media
              ...
              {/* Just enough shade for the name to read over any artwork. */}
              <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/70 to-transparent" />

              <h2 className="pointer-events-none absolute bottom-0 left-0 p-6 text-[clamp(1.3rem,2vw,1.9rem)] font-medium leading-tight tracking-[-0.02em] text-white lg:p-8">
                {project.title}
              </h2>
            </Link>
          ))}
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="relative h-px flex-1 bg-white/12">
              <motion.div
                ...
                className="absolute inset-y-0 left-0 bg-[#e8552f]"
              />
            </div>
          </div>
        </div>
...
function TitleCard() {
  return (
    <div className="flex shrink-0 flex-col justify-center pr-[4vw]" style={{ height: `${FRAME_VH}vh` }}>
      <h1 className="text-[clamp(3rem,5.6vw,5.5rem)] font-semibold leading-[0.95] tracking-[-0.045em] text-white">
        {site.name}
      </h1>
      <p className="mt-5 font-mono text-[11px] uppercase tracking-[0.24em] text-[#e8552f]">
        {site.role}
      </p>
    </div>
  );
...
function EndCard() {
  return (
    <div className="flex w-[24vw] shrink-0 flex-col justify-center" style={{ height: `${FRAME_VH}vh` }}>
      <Link
        href="/reel/contact"
        className="group inline-block text-[clamp(1.8rem,2.6vw,2.6rem)] font-semibold leading-tight tracking-[-0.035em] text-white"
      >
        Get in touch
        <span className="mt-2 block h-px w-full origin-left scale-x-0 bg-[#e8552f] transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-x-100" />
      </Link>
    </div>
  );
}
...
function Stack({ projects }: { projects: Work[] }) {
  return (
    <div className="px-5 pb-24 pt-28 sm:px-8">
      <h1 className="text-[clamp(2.4rem,10vw,4rem)] font-semibold leading-[0.98] tracking-[-0.04em] text-white">
        {site.name}
      </h1>
      <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.24em] text-[#e8552f]">
        {site.role}
      </p>
      ...
      <div className="mt-14 space-y-12">
        {projects.map((project, i) => (
          ...
          >
            <Link href={`/reel/work/${project.slug}`} className="group block">
              <div className="relative overflow-hidden">
                <Media media={project.poster} sizes="100vw" priority={i === 0} className="w-full" />
                <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/70 to-transparent" />
                <h2 className="absolute bottom-0 left-0 p-5 text-2xl font-medium tracking-[-0.02em] text-white">
                  {project.title}
                </h2>
              </div>
            </Link>
